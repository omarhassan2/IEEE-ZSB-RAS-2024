...
static const uint8 * USART_pu8SendData = NULL ;

/*Global variable to carry the Receive Data*/
static uint8 * USART_pu8ReceiveData = NULL ;

/*Global variable to carry the buffer size*/
static uint8 USART_u8BufferSize ;
...
static uint8 USART_u8State= IDLE ;
...
void USART_voidInit (void)
{
	...
	*/
	UBRRH = (uint8)(Local_u16UBRR >> 8) ; /* MSBs at Local_u16UBRR */
	...
	/*Data Size*/
		...
		CLEAR_BIT(Local_u8UCSRC , UCSRC_UCSZ0) ;
		CLEAR_BIT(Local_u8UCSRC , UCSRC_UCSZ1) ;
	...
	/*Clock Polarity (for SYNCHRONOUS mode only)*/
			...
			CLEAR_BIT(Local_u8UCSRC , UCSRC_UCPOL) ;
...
uint8 USART_u8ReceiveBufferSynch(uint8 * Copy_pchString , uint32 Copy_u32BufferSize)
{
	uint8 Local_u8ErrorState = OK ;
...
uint8 USART_u8ReceiveBufferAsynch(uint8 * Copy_pchString , uint32 Copy_u32BufferSize , void (* NotificationFunc)(void))
{
	uint8 Local_u8ErrorState = OK ;
